...
`--max-records` / GUI 里的 `最多扫描条数` 指的是筛选前最多读取多少条原始记录，不是最终 Excel 一定会导出多少条。最终导出数量会再经过城市、关键词和发布日期范围筛选。如果数据源本身没有条数上限，就不用额外说明限制；如果数据源/API 有上限，工具会在摘要 JSON 里记录扫描上限和导出数量。
